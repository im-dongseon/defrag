...
- **주제 폴더 누락 및 번호 금지**: 프로젝트 루트(예: `30_Project-X/`)에 파일을 직접 두지 마십시오. 반드시 `참고자료`, `회의록` 등 하위 폴더를 생성하십시오. **하위 주제 폴더명에는 `[번호]_` 접두사를 절대 붙이지 마십시오.** (예: `10_Category-A/Subject-A/` (O), `10_Category-A/10_Subject-A/` (X))

- **바이너리 처리 순서 위반**: Sidecar(.md) 생성 전에는 원본을 Archive로 이동할 수 없습니다. 추출 실패 시에도 `status: idea` 노드를 선행 생성하십시오.
- **아카이브 경로 부정합**: `91_Raw_Archive/assets/` 하위 폴더명에 `[번호]_`를 누락하지 마십시오. (예: `assets/Project-X/` (X), `assets/30_Project-X/` (O))
