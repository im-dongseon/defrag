...
- **전역 디렉토리 구조 및 아카이브 정합성 검사:** 
  - `10-19`, `20-29`, `30-39` 하위 폴더가 `[번호]_` 패턴(예: `10_`, `20_`, `30_`)으로 시작하는지, 그리고 모든 파일이 주제 폴더 내부에 위치하는지 점검합니다.
  - **[Archive 정합성]:** `91_Raw_Archive/assets/` 하위의 도메인 폴더명이 위키의 메인 폴더명(`[번호]_[이름]`)과 100% 일치하는지 검사합니다. (`[번호]_`가 누락된 폴더 발견 시 즉시 `[Log] 아카이브 경로 위반: 경로` 기록 후 정정 유도)
  - 위반 시 `[Log] 구조 위반: 경로` 기록 후 규칙에 맞게 재배치를 유도합니다.
...
**바이너리 원본 이주 후 `location` 및 인덱스 갱신 의무:**  
바이너리 원본을 Archive로 이주한 직후, 해당 원본과 연결된 Sidecar 노트의 **`location`** 경로를 임시값(`00_Inbox/unsorted/파일명`)에서 최종 Archive 경로를 포함한 **Obsidian URI**(URL Encoded)로 즉시 갱신합니다. 또한, 재배치(`mv`) 완료 후에는 **Obsidian CLI를 사용하여 볼트 인덱스를 갱신**(`obsidian eval`)하여 데이터 정합성을 확보하십시오.
